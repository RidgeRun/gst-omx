...
#include "gstrrh264parser.h"

GST_DEBUG_CATEGORY_STATIC (gst_rr_h264_parser_debug);
#define GST_CAT_DEFAULT gst_rr_h264_parser_debug
...
#define NAL_LENGTH 4
#define GST_RR_H264_PARSER_SINGLE_NALU_DEFAULT  FALSE
...
#define gst_rr_h264_parser_parent_class parent_class
G_DEFINE_TYPE (GstRrH264Parser, gst_rr_h264_parser, GST_TYPE_BASE_TRANSFORM);

static void gst_rr_h264_parser_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_rr_h264_parser_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static GstCaps *gst_rr_h264_parser_transform_caps (GstBaseTransform * btrans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter);
static gboolean gst_rr_h264_parser_set_caps (GstBaseTransform * btrans,
    GstCaps * incaps, GstCaps * outcaps);
static GstFlowReturn gst_rr_h264_parser_transform_ip (GstBaseTransform * trans,
    GstBuffer * buf);

static void
gst_rr_h264_parser_class_init (GstRrH264ParserClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SINGLE_NALU,
      g_param_spec_boolean ("single-nalu", "Single NAL Unit ",
          ...
          GST_RR_H264_PARSER_SINGLE_NALU_DEFAULT, G_PARAM_READWRITE));

  base_transform_class->transform_caps =
      GST_DEBUG_FUNCPTR (gst_rr_h264_parser_transform_caps);
  base_transform_class->set_caps =
      GST_DEBUG_FUNCPTR (gst_rr_h264_parser_set_caps);
  base_transform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_rr_h264_parser_transform_ip);

  gst_element_class_set_static_metadata (element_class,
      "H.264 parser element", "Codec/Parser/Converter/Video",
...
static void
gst_rr_h264_parser_init (GstRrH264Parser * self)
{
  self->header_size = 0;
  self->set_codec_data = FALSE;
  self->single_nalu = GST_RR_H264_PARSER_SINGLE_NALU_DEFAULT;
}

static void
gst_rr_h264_parser_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstRrH264Parser *self = GST_RR_H264_PARSER (object);
...
static void
gst_rr_h264_parser_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstRrH264Parser *self = GST_RR_H264_PARSER (object);
...
static GstCaps *
gst_rr_h264_parser_caps_change_stream_format (GstCaps * caps,
...
static GstCaps *
gst_rr_h264_parser_transform_caps (GstBaseTransform * btrans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
  GstCaps *tmp, *tmp2;
  GstCaps *result;
  ...
  tmp = gst_rr_h264_parser_caps_change_stream_format (caps, direction);
...
static gboolean
gst_rr_h264_parser_fetch_header (guint8 * data, gint buffer_size,
...
static gboolean
gst_rr_h264_parser_get_codec_data (GstRrH264Parser * self, GstBuffer * buf,
    GstBuffer ** codec_data)
{
  ...
  gst_rr_h264_parser_fetch_header (header, info.size, &sps, &pps,
      &self->header_size);
...
static gboolean
gst_rr_h264_parser_set_codec_data (GstRrH264Parser * self, GstBuffer * buf)
{
...
static gboolean
gst_rr_h264_parser_to_packetized (GstRrH264Parser * self, GstBuffer * buffer)
{
...
gst_rr_h264_parser_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  GstRrH264Parser *self = GST_RR_H264_PARSER (trans);
  ...
  if (!gst_rr_h264_parser_to_packetized (self, buf))
...
static gboolean
rr_h264_parser_init (GstPlugin * self)
{
  /* debug category for fltering log messages
   */
  GST_DEBUG_CATEGORY_INIT (gst_rr_h264_parser_debug, "rrh264parser",
      0, "RR H.264 parser");

  return gst_element_register (self, "rrh264parser", GST_RANK_NONE,
      GST_TYPE_RR_H264_PARSER);
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    ...
    rr_h264_parser_init, VERSION, "LGPL", PACKAGE_NAME, GST_PACKAGE_ORIGIN)
